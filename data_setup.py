...
# define funciton with gather as list of features we are interested in 
				...
				for file in each_file:
					...
					try:
							...
							for tag in soup.findAll('b'):
								if tag.parent.name == 'big':
									if tag.text != "Key Statistics":
										...
										stock_price = float(re.search(r'(\d{1,8}\.\d{1,8})',stock_price))
										stock_price = float(stock_price.group(1))	
						except Exception as e:
							# tag = soup.find_all('span', {'class':'time_rtq_ticker'})
							# print(tag)
							# stock_price = float(tag.firstChild().text) 
							# print(stock_price)
						...
						if value_list.count("N/A") > 0:
							...
							# append scraped data values to panda dataframe
